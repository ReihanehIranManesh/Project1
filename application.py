...
def home():
    ...
    elif request.method == "GET" or searchinput == "" :
       return render_template("home.html", results = "Nothing", username = username[0])

    return redirect(url_for('search', kyw = searchinput))


@app.route("/search?keyword=<string:kyw>", methods=["GET", "POST"])
def search(kyw):
    id = session["user_id"][0]
    username = db.execute("SELECT username FROM users WHERE id = :id", {"id": id}).fetchone()
    searchinput = kyw
...
def submitreview(title):
    ...
    elif db.execute("SELECT * FROM reviews WHERE user_id = :user_id AND book_id = :book_id",
    {"user_id": user_id[0], "book_id" : book_id[0]}).rowcount != 0 :
